...
function ActivityLogPage() {
  ...
  const imports = useInfiniteQuery(
    buildInfiniteQuery(["activity", "import"], async (from, to) => {
      if (canUseLocalData()) {
      ...
      const { data, error } = await supabase
        .from("import_logs")
        .select(
          "id, file_name, status, imported_rows, total_rows, invalid_rows, error_message, created_at, notes",
        )
  ...
  const exports = useInfiniteQuery(
    buildInfiniteQuery(["activity", "export"], async (from, to) => {
      if (canUseLocalData()) {
        const rows = await localQuery<Record<string, unknown>>(
      ...
      const { data, error } = await supabase
        .from("export_logs")
        .select(
          "id, export_type, format, tables, row_count, status, error_message, created_at, notes",
        )
        .order("created_at", { ascending: false })
        .range(from, to);
      if (error) throw error;
  ...
  const prices = useInfiniteQuery(
    buildInfiniteQuery(["activity", "price"], async (from, to) => {
      if (canUseLocalData()) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const configs: Array<{
        table: "import_logs" | "export_logs" | "price_history";
        key: string;
        label: string;
      }> = [
      ...
      for (const c of configs) {
        ...
        const ch = supabase
          ...
          .on(
            "postgres_changes",
            { event: "INSERT", schema: "public", table: c.table, filter: `user_id=eq.${uid}` },
            (p) => {
              const row: any = p.new;
  ...
  const importRows = useMemo(
    () => (imports.data?.pages ?? []).flatMap((p) => p.rows),
    [imports.data],
  );
  const exportRows = useMemo(
    () => (exports.data?.pages ?? []).flatMap((p) => p.rows),
    [exports.data],
  );
  ...
  const entries = useMemo<UnifiedEntry[]>(() => {
    ...
    for (const l of importRows) {
      out.push({
        ...
        detail:
          `${formatNumber(l.imported_rows)} / ${formatNumber(l.total_rows)} صف` +
    ...
    return out
      ...
      .filter(
        (e) =>
          status === "all" || e.status === status || (status === "success" && e.status === "info"),
      )
  ...
  return (
    <AppShell
      ...
      subtitle="استيراد · تصدير · تغييرات الأسعار — تحديث لحظي"
      showBack
    >
        ...
        {anyLoading ? (
          ...
          <div className="text-center py-8 text-sm text-muted-foreground">
            ...
            <div className="mt-3 flex justify-center gap-2 text-xs">
              <Link to="/import" className="text-brand hover:underline">
                صفحة الاستيراد
              </Link>
              <span>·</span>
              ...
              <span>·</span>
              <Link to="/price-history" className="text-brand hover:underline">
                سجل الأسعار
              </Link>
            </div>
          </div>
              ...
              {entries.map((e) => {
                ...
                return (
                  <li key={e.id} className="rounded-lg border p-2.5 text-xs">
                    ...
                    {e.meta && (
                      <div
                        className={`mt-0.5 text-[10px] ${e.status === "failed" ? "text-rose-600" : "text-muted-foreground"}`}
                      >
                        {e.meta}
                      </div>
                    )}
