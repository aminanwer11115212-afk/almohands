...
function PriceHistoryPage() {
  ...
  const filtered = useMemo(() => {
    let list = rows;
    if (q.trim())
      list = list.filter((r) => (r.products?.name || "").toLowerCase().includes(q.toLowerCase()));
    if (dir !== "all")
      list = list.filter((r) =>
        dir === "up"
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="text-xs text-muted-foreground">
            ...
            <input
              ...
              className="w-full h-10 rounded-xl border border-border bg-background px-2 text-sm nums"
            />
          </label>
          <label className="text-xs text-muted-foreground">
            <span className="flex items-center gap-1 mb-1">
              ...
              return (
                <li key={r.id} className="bg-card rounded-xl border border-border p-3 shadow-sm">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <div className="font-bold text-sm truncate">
                        {r.products?.name || "منتج محذوف"}
                      </div>
                      <div className="text-[11px] text-muted-foreground">
                        {new Date(r.created_at).toLocaleString("ar-EG")} ·{" "}
                        {r.source === "purchase" ? "من فاتورة شراء" : "تعديل يدوي"}
                      </div>
                    </div>
                    <div
                      className={`flex items-center gap-1 text-xs font-bold ${up ? "text-rose-600" : "text-emerald-600"}`}
                    >
                  ...
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    <span className="text-muted-foreground line-through nums">
                      {formatSDG(Number(r.old_price))}
                    </span>
                    <span className="text-muted-foreground">→</span>
                    <span className="font-bold nums">{formatSDG(Number(r.new_price))}</span>
                    <span
                      className={`ms-auto nums ${diff >= 0 ? "text-rose-600" : "text-emerald-600"}`}
                    >
                      {diff >= 0 ? "+" : ""}
                      {formatSDG(diff)}
                    </span>
...
function StatCard({
  ...
  const c =
    tone === "emerald" ? "text-emerald-600" : tone === "rose" ? "text-rose-600" : "text-brand";
  return (
